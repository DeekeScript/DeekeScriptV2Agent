...
目标：先连机 → 边写边同步 → 写完必须验证。禁止「只生成代码、不验证」就结束。设备已连上时，禁止用「声明未验证」代替调试。

## 什么算已验证（tasks）

**算：** 已 `launch` 目标 App；`snapshot` 或等价节点树对照过选择器；片段 `run` 打出目标页关键节点的 `text`/`desc`/`bounds`；点击或输入按清单验证过；再 `run-file`（可用数量=1）。回复里须引用这些 logs。
...
3. 操作第三方 App：写选择器前必须 snapshot（或 /ai/nodes），禁止凭记忆猜 desc/id
4. 落盘（入口、页面、tasks、common…）
5. 每改一批文件：write 同步到手机（不要攒到最后）
6. 验证：界面可预览/点测；任务先 run 片段（目标 App），再 write → run-file → 读 logs
7. 失败则修代码 → 再 write → 再验；通过后交付
```

连不上设备时：仍按契约生成完整可运行代码，并明确列出用户须开启的权限与地址；**不得假装已实机验证**。`status` 已通则必须走上面第 3、6 步，不得改口「请用户自己运行」。
...
| 3 | **写完必须验证**。界面与脚本都要验。任务禁止一上来整段盲跑，先片段后整体；也禁止只测配置就不进目标 App。 |
...
| 节点歧义或界面与假设不符 | 自己先 `snapshot`；对不上再问用户当前界面或要点的文案 |
| 业务规则不清（次数、间隔、停条件） | 具体数值与边界 |
| 连续 3 轮实机仍失败 | 用户确认当前屏幕状态；或提供正确节点特征（text/id/desc） |
| 支付、批量删除、向**用户未要求**的对象发私信 | 仍须先做找节点片段；**整段 `run-file` 前**口头确认 |
| 用户明确要求的搜索 / 点赞 / 评论 / 刷流 | **必须**按片段清单调试；整段可用数量=1 的 `run-file`。不要把「会真实发出」当成不启动目标 App 的理由 |
...
- 用户已经要求搜索 / 点赞 / 评论 / 刷流：自己 `launch` 目标 App 做片段验证，不要先问「能不能调试」  

## 交付前自检

- [ ] 编写前已连机（仅连不上时才声明无法验证，并列出用户侧步骤）  
- [ ] 过程中改动已 `write`  
- [ ] `tasks/*.js`：已在**目标 App / 目标页**片段验证，回复含节点 logs（`text`/`desc`/`bounds`）；不是只测了 Storage/权限  
- [ ] 操作第三方 App：写选择器前已 `snapshot`（或 `/ai/nodes`）  
- [ ] 片段通过后已 `run-file`（可用数量=1）；`logs` 无未处理错误；循环有上限  
