...
细节：[心智模型](./mental-model.md)、[目录](./project-layout.md)、[Rhino](./rhino.md)、[上下文边界](./context-split.md)。编写 `tasks/*.js` 时另守 [`automation-loop.md`](./automation-loop.md)。
...
| 10 | **默认不写** `floatWindow` / `menus`。未配时连点两次停止（第一次变关闭图标，**3 秒内**再点）。用户要自定义菜单时：`menus` 最多 5 个；每项须在 `FloatWindow.on` 绑定，与 JSON **同一轮**生成。见 [`float-window.md`](../03-recipes/float-window.md)。 |
...
| 14 | 禁止在悬浮窗菜单回调里用 `Engines.closeAll()` 停任务（无效）。手动停用 `FloatWindow.stopTask()`。 |
...
| 手动停（有 menus） | `FloatWindow.stopTask()` | 菜单里 `Engines.closeAll()` |
