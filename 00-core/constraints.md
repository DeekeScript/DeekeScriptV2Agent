...
| 16 | **任务验证完成条件**（设备 `status` 正常）：`tasks/*.js` 必须在目标 App / 目标页跑过片段 `run`，回复中须有关键节点 `text`/`desc`/`bounds` 的 logs。**「片段」= 单个动作**，不是整段任务少跑几条。各分段通过后才 `run-file` 做验收（可用数量=1）。操作第三方 App：写选择器前必须按 [`app-survey.md`](../02-script/pitfalls/app-survey.md) 调研；**每种形态打开后带图 `snapshot`（看布局）+ 节点 logs（写选择器）**。禁止用「能不截图就不截图 / 省 token」跳过看屏。分段测见 [`segment-test.md`](../02-script/pitfalls/segment-test.md)。图色任务用 `Images.*` 打结果日志。仅 `write`、仅权限、仅 `Storage` **不算**已调试。用户任务本身要操作第三方 App 时，启动目标 App 做调研和分段验证**不必再问**。细节见 [`device.md`](./device.md)。 |
...
| 15 | 禁止把 `status`、`Storage` 读写、`require` 能加载、本工程 UI 预览、**只看截图不打节点 logs**、**只拉节点不看屏**当作 `tasks/*.js` 已验证。 |
...
| 任务已验证 | 先调研（带图看屏 + 节点）；分段 `run`；再 `run-file` 验收 | 只测 Storage / 权限 / write；或不看屏；或只看图不打 logs |
| 操作第三方 App | 并列形态都摸到；父子层分开；单步分段测；验收才整段 | 只做一种形态；子项当父项；一上来 `run-file`；或以「会真实发出」为由不启动目标 App |
| 写选择器前 | 每种形态打开：带图 snapshot 看布局 + 拉节点 | 凭记忆猜 desc；只 snapshot 当前碰巧打开的那一页；不看屏只靠节点树猜布局 |
