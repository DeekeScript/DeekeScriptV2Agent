...
只跑脚本时，根目录仍必须有 [`deekeScript-v2.json`](../01-ui/entry-json.md)（插件靠它识别工程），不必打开任何页面。

## JSON 只管 UI

| 文件 | 职责 | 禁止 |
|------|------|------|
| `deekeScript-v2.json` | 识别工程；有界面时写 `homePage`、`window`、`pages`、`bottomMenus`、`floatWindow` | 写业务循环、无障碍操作 |
