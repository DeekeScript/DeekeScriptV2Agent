...
依赖：[`permission.md`](permission.md)、[`code-org.md`](code-org.md)、[`ui-and-task.md`](ui-and-task.md)、[`require.md`](require.md)、[`UiSelector.md`](api/UiSelector.md)、[`device.md`](../00-core/device.md)。操作第三方 App 前先读 [`app-survey.md`](pitfalls/app-survey.md)、[`progress-model.md`](pitfalls/progress-model.md)、[`segment-test.md`](pitfalls/segment-test.md)。刷推荐流 / 进主页取号时再读 [`skip-on-item-failure.md`](pitfalls/skip-on-item-failure.md)。
...
以一条内容为进度。进主页弹窗、读不到字段时 **skip 本条并前进到下一条未处理目标**。一屏一条才滑动；一屏多条时先耗尽当前屏。见 [`skip-on-item-failure.md`](pitfalls/skip-on-item-failure.md)、[`progress-model.md`](pitfalls/progress-model.md)。
...
if (!permission.ensureRun()) {
  ...
  while (processed < maxCount) {
    ...
    if (processed < maxCount) {
      page.swipeNext(); // 仅一屏一条时；一屏多条见 progress-model，返回后扫同屏下一条
    }
  }
...
- **写选择器前先调研**：用户原话里的每种形态、每一层列表都要打开拉节点。见 [`app-survey.md`](pitfalls/app-survey.md)。
- **分段测**：点赞只测点赞，停在目标页 `run`；整段 `run-file` 只做验收。见 [`segment-test.md`](pitfalls/segment-test.md)。
- **`while` 里 `continue` 必须递增计数或设 retry 上限**，否则会无限 toast。
- **刷流**：`continue` 若未前进到下一条未处理目标，须有 skip 上限并最终强制前进；进主页单次尝试。一屏多条时不要无条件 `swipe`。见 [`skip-on-item-failure.md`](pitfalls/skip-on-item-failure.md)、[`progress-model.md`](pitfalls/progress-model.md)。
