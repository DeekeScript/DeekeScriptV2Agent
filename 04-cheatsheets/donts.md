...
| 写 `host` / `debug` / `apis`，或入口文件名不是 `deekeScript.json` | 入口只有 `deekeScript.json`，字段见 [`entry-json.md`](../01-ui/entry-json.md) |
...
| 照抄展厅 Demo 的 `host` / `hint` / 默认 menus / 错误入口文件名 | 见 [`demo-gallery.md`](../00-core/demo-gallery.md) |
