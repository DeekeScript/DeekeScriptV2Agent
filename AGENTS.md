...
| **悬浮球 / 悬浮窗菜单** | **必读** [`01-ui/capabilities/floatWindow.md`](./01-ui/capabilities/floatWindow.md) + [`03-recipes/float-window.md`](./03-recipes/float-window.md)；`action` 仅 `stop`/`hide`/`start`/`executeScript`；**同一轮输出 JSON menus + `FloatWindow.on`** |
...
- **生成 `floatWindow.menus` 时**：必读 [`01-ui/capabilities/floatWindow.md`](./01-ui/capabilities/floatWindow.md) 与 [`03-recipes/float-window.md`](./03-recipes/float-window.md)。`action` 只能是 `stop` / `hide` / `start` / `executeScript`（不是页面那套 navigate/toast）。凡 `onTap`、跳过、自定义开始：**同一轮**写出 `tasks/*.js` 里的 `FloatWindow.on`，禁止只给 JSON。
