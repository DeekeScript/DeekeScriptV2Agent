...
| 作用 | 保持屏幕常亮；任务运行时旋转；连点两次停止任务 | 同样这三件事；配置了 menus 时可展开菜单定制 |

| 点球 | 第一次变成关闭图标，**3 秒内**再点才停止 | 展开扇形菜单（仅当配置了 `floatWindow.menus` 或 `FloatWindow.setMenus`） |

| 未配 menus 时 | 同上 | **同样**：第一次变关闭图标，3 秒内再点停止（与开发器一致） |
...
| **用户手动**连点开发器球 / 未配 menus 的项目球 | 框架内置 | 第一次变关闭图标，3 秒内再点；无需写 JS |
