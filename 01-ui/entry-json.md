...
创建工程或改首页、底栏、全局窗口、页面/组件注册时读这篇。这个文件必须在项目根目录，文件名只能是 `deekeScript.json`。只跑脚本时用它识别工程，不必打开 `homePage`。做界面时启动经过 Splash 后进入 `homePage`。字段一律按下表写。不要写 `host` / `debug` / `apis` / `hooks` / `groups`。
